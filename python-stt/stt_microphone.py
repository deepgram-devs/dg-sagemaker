#!/usr/bin/env python3
"""
Deepgram SageMaker Microphone Streaming Client

This client captures live audio from your microphone and streams it to a
Deepgram model deployed on SageMaker for real-time transcription.
"""
...
class DeepgramSageMakerMicrophoneClient:
    """
    Client for streaming microphone audio to Deepgram models on AWS SageMaker.

    This client uses PyAudio to capture live microphone input and streams it
    to SageMaker's bidirectional streaming API for real-time transcription.
    """
    ...
    async def stream_microphone_audio(self):
        """Stream microphone audio to SageMaker in real-time"""
        chunk_count = 0
        ...
        try:
            while self.is_active:
                ...
                if not self.audio_queue.empty():
                    ...
                    logger.debug(f"Sent chunk {chunk_count} ({len(audio_chunk)} bytes)")
                else:
                    # Small delay to prevent busy waiting
                    await asyncio.sleep(0.01)

        except Exception as e:
            logger.error(f"Error streaming microphone audio: {e}")
            raise
        finally:
...
async def main():
    """Main function to run the Deepgram microphone streaming client"""
    parser = argparse.ArgumentParser(
        description="Stream microphone audio to Deepgram on SageMaker for real-time transcription"
    )
    parser.add_argument(
        "endpoint_name",
    ...
    print("=" * 60)
    print("Deepgram SageMaker Microphone Streaming Client")
    print("=" * 60)
    ...
    # Create client
    client = DeepgramSageMakerMicrophoneClient(
