...
| Streaming client **hangs** while `Invocation4XXErrors` / `InvocationModelErrors` rises | historically SageMaker withheld the 424 until the input stream closed (observed Sept 2026; no longer reproduced as of 2026-09-16 on either client generation) | close the input stream to force the error out, or read the container log; report to Deepgram if it recurs. |
